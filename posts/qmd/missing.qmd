...
A hash is a unique string generated by the content of a file. If the file changes, so does the hash. Git uses this fact to store files efficiently: only the files that have changed between get a new hash, while old files can still be accessed using the same hash value.
