...
subtitle: My takeaways from MIT's "Missing Semester of Your CS Education" course.
format: 
  html:
...
Here's what I picked up on from the 2020 offering of the excellent [./missing-semester](https://missing.csail.mit.edu/) course.

The document is written as a reference so the discussion should be relatively self-contained.
...
Debuggers are programs that let you step through code interactively and re-run sections so you can figure out what the code is doing.

`ipdb`[^6] is like Python's built-in `pdb` but includes niceties such as colored output, tab completion and so on. You start `ipdb` by running `python -m ipdb [script]`.
